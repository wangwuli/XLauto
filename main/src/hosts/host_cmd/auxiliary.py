...
from main.models.models import ScriptFileExecuteEvent, db
...
# 连接单台服务器，单台服务器脚本统一处理
def run_script_worker(info_dict, script_list, time_out, xlauto):
    with xlauto.app_context():
        try:
            ...
            for script_list_one in script_list:
                ...
                # ssh.put_file_exec(script_list_one['script_file_path'], os.path.join('/tmp', file_name), '', type='sh', nohup=False)
                ...
                for i in range(int(time_out / 10)):
                    ...
                    # data = db.session.query(ScriptFileExecuteEvent).all()
